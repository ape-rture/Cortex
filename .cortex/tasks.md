...
- **Research Dribbble and Mobbin for UX/design inspiration workflows** -- Agent: claude -- Explore [Dribbble](https://dribbble.com/) and [Mobbin](https://mobbin.com/) — evaluate how they can feed into the design process (UI patterns, inspiration, reference boards). Document findings in `research/`.
...
- **Unified Capture Store (Phase 9) — Codex phases 1-3 + cleanup** -- Agent: codex -- Branch: `codex/unified-captures-phase9` (ready to merge). Implemented unified capture model on `Task` (`capture_type` + capture metadata), added `MarkdownTaskQueue.listByType()`, and updated markdown parsing/serialization for queue-backed captures. Reworked `/capture`, Telegram triage, Telegram queue ingest, `/content` idea commands, content scanner, and `/gm` content counts to use the unified queue-backed capture path. Slimmed `ContentStore` by removing `ContentIdea`; ideas now live in `actions/queue.md` with `capture_type: content`. Added compatibility updates for scheduler/tests and exported new capture types. Cleanup included removing obsolete empty files `projects/feature-proposals.md`, `projects/ideas.md`, `projects/content-ideas.md` and updating `context/orchestrator.json` content-scanner read permissions to `actions/queue.md`. Validation: `npm run typecheck` and `npm run test:unit` pass (resume-token tests stabilized with relative timestamps).
